...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['coordinates_submit'])) {
    $_SESSION['longitude'] = $_POST['longitude'];
    $_SESSION['latitude'] = $_POST['latitude'];
    echo "<script>alert('Coordinates saved successfully!'); window.location.href='add_miller2.php';</script>";
    exit;
}

// Insert miller data into DB
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_miller'])) {
    $miller = $_POST['miller'];
    $miller_name = $_SESSION['miller_name'];

    $stmt = $con->prepare("INSERT INTO millers (miller_name, miller) VALUES (?, ?)");
    if ($stmt) {
        $stmt->bind_param("ss", $miller_name, $miller);
        if ($stmt->execute()) {
            echo "<script>alert('Miller added successfully!'); window.location.href='add_miller2.php';</script>";
        } else {
...
<head>
    ...
    <style>
        ...
        .step-actions {
            ...
            margin-top: 20px;
            gap: 100px; /* Adds space between the buttons */ s
        }
        button {
            padding: 10px 20px;
            background: #a45c40;
    ...
    <div class="container">
        ...
        <div class="form-content">
            ...
            <form method="POST">
                ...
                <button type="submit" name="coordinates_submit">Save Coordinates</button>
            </form><br>

            <!-- Add Miller Form -->
            <div class="miller-form">
                <form method="POST">
            ...
            <!-- Next Button -->
            <form method="POST" class="step-actions">
