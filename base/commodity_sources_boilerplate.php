...
<script>
// Function to update items per page (re-uses existing logic)
...
// Delete selected sources function (modified for commodity sources)
function deleteSelectedSources() {
    ...
    if (confirm(`Are you sure you want to delete ${selected.length} selected commodity source(s)? This action cannot be undone.`)) {
        ...
        // --- AJAX call to a PHP script to handle deletion ---
        fetch('delete_commodity_sources.php', { // You'll create this file
            method: 'POST',
            headers: {
        ...
        .catch(error => {
            ...
            alert('An error occurred while deleting commodity sources.');
        });
    }
}

// Export selected sources function (placeholder, modified for commodity sources)
function exportSelectedSources(format) {
    ...
    // --- You would typically make an AJAX call here or redirect to a generate_export.php ---
...
// Basic filter logic (similar to your filter.js but directly in this script for simplicity)
document.addEventListener('DOMContentLoaded', function() {
    const table = document.getElementById('commoditySourceTable');
    const rows = table.getElementsByTagName('tr');

    const filterInputs = {
        id: document.getElementById('filterId'),
        admin0: document.getElementById('filterAdmin0'),
        admin1: document.getElementById('filterAdmin1'),
    ...
    function applyFilters() {
        ...
        for (const key in filterInputs) {
            filterValues[key] = filterInputs[key].value.toLowerCase();
        }
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            // Skip rows without enough cells (e.g., "No sources found" message)
            if (cells.length < 5) continue; 

            const id = cells[1].textContent.toLowerCase();
            const admin0 = cells[2].textContent.toLowerCase();
            const admin1 = cells[3].textContent.toLowerCase();
            const createdAt = cells[4].textContent.toLowerCase();

            let match = true;

            if (filterValues.id && !id.includes(filterValues.id)) {
                match = false;
            }
            if (filterValues.admin0 && !admin0.includes(filterValues.admin0)) {
                match = false;
            }
            if (filterValues.admin1 && !admin1.includes(filterValues.admin1)) {
                match = false;
            }
            if (filterValues.createdAt && !createdAt.includes(filterValues.createdAt)) {
                match = false;
            }
    ...
    for (const key in filterInputs) {
        if (filterInputs[key]) { // Check if the element exists
            filterInputs[key].addEventListener('keyup', applyFilters);
        }
    }
});
</script>
